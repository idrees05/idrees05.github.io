...
@app.get("/files/{r2_key:path}")
async def serve_local_file(r2_key: str, request: Request):
    """Serve locally stored uploads (dev only — skipped when R2 is configured)."""
    from storage import r2_configured, local_file_path
    if r2_configured():
